...
The most important part of a Carbon script is the script's metadata. Metadata
is declared as a variable called *carbon*. The
...
.#!/usr/bin/env groovy
****
Are you wondering why the previous script had `#!/usr/bin/env` at the begginning ? Check <<Executable Scripts>> section
****
...
At some point your script metadata may be a little bit bigger
than you expected.
...
There're more lines of metadata than the task we try to
achieve. It's a lot of noise. Wouldn't be nice to move all that
metadata stuff somewhere else ? What about this:
...
metadata within the same script requires to use the *carbon*
variable and use a map object, when using an external metadata
file, all properties should appear outside the carbon
variable.
...
yaml file, and then consumed from your script as a plain map simply
