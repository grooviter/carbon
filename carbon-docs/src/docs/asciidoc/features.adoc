...
contain a set of basic features that try to help developers to deal
...
There're plenty of scenarios where we could find ourselves, dealing with
local files right ? Among other functions Carbon already has an integration with
Apache Ant which covers most of the operation that we may require when
...
[source, groovy]
----
include::{examples}/logging/lines.groovy[indent=0]
----
...
.default execution
----
include::{examples}/sql/h2-test.gsh[indent=0]
----

[source, groovy]
.External configuration
----
include::{examples}/sql/h2-test.yaml[indent=0]
----
...
In order to use some ansi colors when writing to the console you can
use the method *ansi()* available in any Carbon script:

[source, groovy]
.ansi
----
include::{examples}/ansi/ansi-test.gsh[indent=0]
----

<1> Clears the console
<2> Shows a black underlined text with yellow background
<3> Capital letters in bold and in yellow
<4> A banner blinking slowly

Using the pattern *@|color text|@* you can color any text you want
when logging your app.

And of course, you can use ansi colors in your script
configuration. The following example has a banner in its header, How
cool is that ?

[source, groovy]
.Script
----
include::{examples}/ansi/banner.gsh[indent=0]
----

A[source, groovy]
.Script configuration with banner in header
----
include::{examples}/ansi/banner-cfg.groovy[indent=0]
----
